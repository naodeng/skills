...
├── site/assets/app.js
├── site/assets/site-icon.svg       # when branded icons are part of the project
├── site/assets/favicon-32.png       # when branded icons are part of the project
├── site/assets/apple-touch-icon.png # when branded icons are part of the project
...
When the supplied reference project has an established site template, reuse its responsibilities: a shared `html_page()` wrapper, external CSS and JS, brand/favicon assets when present, consistent header and footer, homepage search/filter controls, phase navigation, lesson cards, print behavior, and previous/next lesson links. Change the title, topics, phase names, and explanatory copy to the requested subject, but keep the interaction and asset architecture recognizable. Analytics may be optional, but if included it must use a CI secret or environment variable rather than a copied token.
...
6. copy or reference shared assets from `site/assets/`, including icon assets when the selected site contract requires them;
