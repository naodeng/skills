...
6. Keep shared presentation in `site/assets/` and keep generated HTML in `_site/`. The generator should link the external CSS and JS assets instead of embedding a one-off replacement stylesheet. Never hand-edit generated pages. Preserve the reference site's accessible headings, links, focus states, responsive layout, print behavior, search/filter behavior, header/footer, and lesson navigation.
   - When a reference site uses the Explore-inspired visual language, carry it into the generated site as a reusable visual direction: ASCII/code-native labels, restrained editorial typography, a warm paper base, diffuse teal/amber atmosphere, fine grid texture, translucent glass panels, soft borders, and purposeful hover elevation.
   - Apply the direction consistently to the homepage hero/metrics, phase lesson cards, sidebar filters, lesson article shell, metadata pills, tables, code blocks, and previous/next navigation. Keep the content hierarchy and interaction semantics readable; glass effects support the learning interface and must not replace clear contrast or stable selectors.
   - Prefer CSS variables and shared selectors in `site/assets/style.css`; keep the generator responsible for structure and `app.js` responsible for behavior. Use progressive enhancement for `backdrop-filter`, preserve visible focus states, and add a mobile override for card grids, article padding, code blocks, and lesson navigation.
   - Treat this as an adaptable style contract, not a pixel-for-pixel copy of one repository. Reuse the visual principles only when they fit the requested topic and reference site; do not copy project-specific content, URLs, tokens, or generated HTML.
